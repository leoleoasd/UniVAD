...
def get_area_only_histo(testlogicalfiles, subimage, k_offset, dbscan, nn):
    ...
    for imgfilepath in testlogicalfiles:
        ...
        ret2, thresh2 = cv2.threshold(
            gray_img, int(k_offset * ret), 1, cv2.THRESH_BINARY
        )
...
def test_select_binary_offsets(
    testfiles, subimage, k_offset, mean, std, cmean=0, cstd=0
):
    arealist = []
    colorlist = []
    for imgfilepath in testfiles:
        ...
        color_sum_a = imglabo[:, :, 1].astype(np.float16)
        color_sum_b = imglabo[:, :, 2].astype(np.float16)
        color_div = (color_sum_b / (color_sum_a + 0.0000001)) * (
            color_sum_b / (color_sum_a + 0.0000001)
        )
    ...
    color_numpy_return[np.isinf(color_numpy_return)] = 1.0
